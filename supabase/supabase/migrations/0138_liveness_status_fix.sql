-- 0138_liveness_status_fix.sql — corrective: liveness_run v4. The event
-- eligibility filter used status = 'published', but events in production use
-- status = 'active' (verified against the live schema's own queries), so the
-- occurrence generator matched zero real events and Event Pulse sat idle.
-- v4 is v3 with that single predicate corrected — everything else identical.
-- Run this AFTER 0129–0137, then Admin → Event Pulse → Run now.
