-- 0147_courts_finder.sql — the map-based court finder (Courts page rebuild).
--
-- FACILITY COLUMNS the new UI depends on (hide-when-null, never fake):
--   indoor      — false by default; a trigger enforces indoor ⇒ lights = true
--   lights/free — NULLABLE tri-state: null = unknown → the chip is hidden and
--                 the filter only matches explicit true
--   court_count — playable surfaces; null = unknown → meta segment hidden
--   confirmed_at/by — the "screened by Klimr" guarantee
--
-- courts_finder(lat, lng, radius_mi): ONE set-based pass returning distance,
-- member-review aggregates (Google rating stays as fallback in the row),
-- live-queue state, 90-day player stats from court_checkins, and the busy
-- band. Queue↔venue linkage: a session belongs to a court when its event is
-- at that court OR its center sits within ~0.15 mi. Busy bands compare the
-- current hour-of-week against the court's OWN 8-week distribution across all
-- 168 slots; under 12 lifetime linked matches the band is NULL (omit, don't
-- guess). SECURITY DEFINER (queue tables aren't member-readable). Idempotent.

alter table public.courts add column if not exists indoor boolean not null default false;
alter table public.courts add column if not exists lights boolean;
alter table public.courts add column if not exists free boolean;
alter table public.courts add column if not exists court_count int;
alter table public.courts add column if not exists confirmed_at timestamptz;
alter table public.courts add column if not exists confirmed_by uuid;

do $$ begin
  alter table public.courts add constraint courts_court_count_check
    check (court_count is null or (court_count >= 1 and court_count <= 200));
exception when duplicate_object then null; end $$;

create or replace function public.courts_normalize_indoor()
returns trigger language plpgsql as $$
begin
  if new.indoor then new.lights := true; end if;
  return new;
end $$;

drop trigger if exists courts_indoor_lights on public.courts;
create trigger courts_indoor_lights
  before insert or update of indoor on public.courts
  for each row execute function public.courts_normalize_indoor();

create index if not exists courts_lat_lng_idx on public.courts (lat, lng) where lat is not null;
create index if not exists court_checkins_court_created_idx on public.court_checkins (court_id, created_at desc);

create or replace function public.courts_finder(
...
linked_sessions as (
  ...
  join court_sessions s
    on (
      exists (select 1 from events e where e.id = s.event_id and e.court_id = h.id)
      or (
