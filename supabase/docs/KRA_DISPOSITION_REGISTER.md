...
| KRA-018 | P1 | M | 053 | ● | Storage backup DR | **FIXED — Production-verified (restore drill EXECUTED+PASSED 2026-08-20: 5m10s, 11/11 checks; RESILIENCE §6 row 1)** | Quarantine copying removed (D-22 as amended) so script and docs agree. Verification extended to **every** destination class and to `rclone check` checksums for the plain buckets — counting cannot tell "the file came back" from "a file with that name came back". **Still owed and unchanged: no run history, no alerting, and NO RESTORE DRILL — blockers B-01/B-02 stand.** |
...
| KRA-027 | P1 | M | 026,029 | ● | PYMK privacy/fail-open | **FIXED — Executed-local, replay-proven (0261 replays green in every from-zero run; latest 297/0 with klimr_ready 43/43)** | 0261 removes `neighborhood` from the return type (not merely from the card) and adds the OD-2 ladder check so a Connect button is never offered to someone who would refuse it. Both fail-open cache paths closed: an unavailable validator now recomputes, and an RPC failure returns an EMPTY rail rather than a stale unvalidated payload. **The migration has not been replayed** — harness unavailable. |
...
**2026-08-18 update:** KFU-011 and KFU-012 EXECUTED and PRODUCTION-CONFIRMED (43-check green verdict) (0292/0293, wpi_admission_suite 13 checks, repro-first). Remaining in the I-series: KCDX-046 residual only.

**2026-08-20 (latest): WP-T EXECUTED — KFU-032 (one deadline predicate + tripwire), 0297 PRODUCTION-CONFIRMED same day (owner post-check 4/4 green: table gone, fns=2, journal 405a86c1, ready 43/43), KFU-027 (this reconciliation + KRA-018/027 upgrades). Replay doctrine rewritten after the boundary-reaper postmortem. 0295 PRODUCTION-CONFIRMED same day (5/5) — the full migration chain 0001–0297 is confirmed with zero pending state.**

**2026-08-20 (WP-T reconciliation): register audited end-to-end. No unsigned DISPUTED or DEFER rows exist — every disposition below is either evidence-backed or listed here. The honestly-open ledger, in one place:**
- **KRA-001** — REOPENED 2026-08-11 (queue/courtside authz reverted after a production incident). Re-landing is its own future batch.
- **KRA-005** — media-safety seam fixed on the Feed path; the remaining upload surfaces (avatars, gallery, listings, chat media) still route around `lib/media-safety.ts`. Breadth owed.
- **KRA-006** — payment-proof object binding landed (0245); the digest capture + admin verify command wiring is still owed.
- **KRA-017** — privileged-audit false-success removed (0246); breadth across the remaining privileged surfaces owed.
**Everything else marked FIXED now carries its evidence level inline; KRA-018 and KRA-027 upgraded this pass with the drill row and the replay verdict as proof.**

**2026-08-20 (later still): WP-U ENGINEERING COMPLETE — lint 137→0 ENFORCED (max-warnings 0; four a11y rules promoted to ERROR per their own comments). KFU-022 (signal reaches the hops, abort proven), 016 (cropper keyboard pan), 017 (dialogs/palette/cards/dropdowns rebuilt on house patterns; keyboard photo-reorder ADDED where none existed), 018 (verified pre-existing CSS-token rails), 023 (assessed satisfied: aria-live ×6, real tables, 9 loading boundaries), 019 folded in. Manual leg = docs/DEVICE_CHECKLIST_WP-U.md, owner-run pending.**

**2026-08-20 (later): WP-R EXECUTED — KFU-025 (CSP enforced single-source with loud fallback; dev-deps 0 vulns; every action digest-pinned via live ls-remote; CycloneDX SBOM artifact), KFU-024 (milli-CLS contract at route+DB+column+view; rum_p75_daily the honest reader; proven live), KFU-035 (checked+loud audit writes; durably fallback awaited), KFU-034 (journal checksums backfilled 0262-0295 nulls_left=0; canonical-zeros convention + verifier; release manifest 1044 files top-hash c1a47a41…). Replay 296/0, ready 43. Remaining in WP-R: staging-class proofs.**

**2026-08-20: KFU-020 CLOSED — first restore drill EXECUTED AND PASSED (owner-run): Restore-to-new-project BETA, 5m10s, drill_check 11/11, klimr_ready 43/43 on the restored copy; RPO/RTO measured, §6 first row written, drill-run claim flipped. Finding: pg_cron restores LIVE incl. outbound heartbeat with baked prod secret — §3 warning added.**

**2026-08-19 (later still): KFU-020 DRILL PREPARED — drill_check_2026-08.sql (read-only single-paste verdict, klimr_ready as star witness, proven on the 295 head); RESILIENCE §3 rewritten two-pathed and two-tiered for a non-engineer; klimr-drill org persists for staging proofs. Drill execution is owner-run; §6 log awaits its first row.**

**2026-08-19 (later): NIGHTLY STORAGE BACKUP repaired — verify direction inverted to one-way (source ⊆ destinations, checksums; cryptcheck for encrypted), retained deletions reported not failed; first post-wipe run had exposed the count-equality contradiction with the additive design. RESILIENCE §1/§5 reconciled to the running workflow; KCDX-053 noted closed by it (drill still open). Shim-proven offline, three scenarios.**

**2026-08-19: WP-H TAIL EXECUTED — KFU-015 (client/server split + server-only guard + enforcing bundle budgets), KFU-014 (coordinates out of URLs; memory-only + POST; legacy ?ll canonicalized away), KFU-021 (resolved-Response classification: 429/5xx feed the breaker and consume retries; caller contract unchanged), KFU-026 (SECURITY.md corrected ×6 incl. two new LIVING claims; .env.example verified clean; RESILIENCE.md verified clean). App/docs only — no migration.**

**2026-08-18 (later still): DIAGNOSTIC PACKET COMPLETE — both parked suites green and hooked (rls 5 checks; social 7 blocks); found and repaired the dead decline cooldown (0295, three-layer regression live since 0238). connection_declines retirement filed under WP-T hygiene.**

**2026-08-18 (later): KCDX-046 residual EXECUTED (0294, four generation/clearing commands, bracket_generation_suite 21 checks, repro-first). I-SERIES CLOSED.**
