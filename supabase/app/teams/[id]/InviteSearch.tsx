...
export function InviteSearch({ teamId, friends }: { teamId: string; friends: Friend[] }) {
  ...
  function invite(id: string) {
    ...
    startTransition(async () => {
      await inviteToTeam(fd);
      setInvited((s) => new Set(s).add(id));
    });
  }

  // No friends to add at all.
  if (friends.length === 0) {
    return (
      <div className="rounded-2xl border border-dashed border-rule bg-surface p-6 text-center">
        ...
        <p className="mt-2 text-sm font-semibold text-ink">Only friends can be added to a team</p>
        ...
        <Link href="/network" className="press mt-3 inline-flex items-center gap-1.5 rounded-lg bg-brand px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-[#B52D0B]-deep">
  ...
  return (
    <div>
      <p className="mb-2 text-xs text-mute">Only players you&rsquo;re friends with can be added to a team.</p>
      ...
      {visible.length === 0 ? (
        <p className="mt-2 text-sm text-mute">
          {q.trim() ? `No friends match “${q.trim()}”.` : "All your friends are already on this team or invited."}
        </p>
      ) : (
