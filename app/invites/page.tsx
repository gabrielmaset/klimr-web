...
import { Inbox, BadgeCheck, Check, X, Clock, Users } from "lucide-react";
...
export default async function InvitesPage({ searchParams }: { searchParams: Promise<{ tab?: string; kind?: string }> }) {
  ...
  const kind: Kind = sp.kind === "friends" || sp.kind === "teams" ? sp.kind : "all";
  ...
  const [{ data: frIn }, { data: frOut }, { data: tiIn }, { data: tiOut }] = await Promise.all([
  ...
  const personIds = [...new Set([...friendInIds, ...friendOutIds, ...(tiOut ?? []).map((t) => t.invited_user_id)])];
  ...
  const counts = {
    received: (frIn ?? []).length + (tiIn ?? []).length,
    sent: (frOut ?? []).length + (tiOut ?? []).length,
  };
  ...
  const nothing =
    (!showFriends || friendCards.length === 0) && (!showTeams || teamCards.length === 0);

  return (
    <div className="mx-auto max-w-2xl px-5 py-8 sm:py-10">
      <div className="mb-5">
        <p className="kicker text-faint">Invites</p>
        <h1 className="font-display text-4xl leading-none text-ink sm:text-5xl">Friends &amp; teams</h1>
      </div>
      ...
      {nothing ? (
        <div className="rounded-2xl border border-rule bg-surface p-10 text-center">
          ...
          <p className="mx-auto mt-3 max-w-sm text-sm text-mute">
            {dir === "received" ? "No pending invites. Friend requests and team invitations will show up here." : "You haven’t sent any pending invites."}
          </p>
        </div>
      ) : (
