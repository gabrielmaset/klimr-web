    ...
    <div className="mx-auto max-w-3xl px-5 py-8 sm:py-10">
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-3">
        <div className="rounded-2xl border border-rule bg-surface p-4">
          ...
          <p className="mt-1 font-display text-2xl text-ink">{members.length}</p>
        </div>
        <div className="rounded-2xl border border-rule bg-surface p-4">
      ...
      {canManage ? (
        <section className="mt-6 rounded-2xl border border-rule bg-surface p-5">
          ...
          <EditTeamForm teamId={team.id} name={team.name} city={team.city ?? ""} neighborhood={team.neighborhood ?? ""} />
        </section>
      ) : null}
