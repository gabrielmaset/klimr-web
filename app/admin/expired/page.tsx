...
export default async function ExpiredContentPage({
  ...
  if (type === "match") {
    ...
    for (const m of data ?? [])
      rows.push({ kind: "Match", id: m.id, title: `${sportMeta(m.sport_key).name} · ${m.format}`, when: m.scheduled_at, organizer: nameOf.get(m.organizer_id ?? "") ?? "—", status: matchDisplayStatus(m.status, m.scheduled_at).label, href: `/play/${m.id}?from=admin` });
  } else if (type === "event") {
    let qb = admin
      .from("events")
