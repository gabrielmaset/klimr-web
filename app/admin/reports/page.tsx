          ...
          {reports.map((r) => (
            <div key={r.id} className="rounded-2xl border border-rule bg-surface shadow-e1 p-5">
              ...
              <form action={resolveReport} className="mt-4 flex flex-wrap items-center gap-2 border-t border-rule pt-3">
                ...
                <select name="status" defaultValue={r.status === "open" ? "reviewing" : r.status} className="rounded-[10px] border border-rule-2 bg-surface px-3 py-2 text-sm text-ink outline-none focus:border-brand focus:ring-4 focus:ring-brand/15">
