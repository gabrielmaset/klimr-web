      ...
      <div className="flex items-center gap-2">
        ...
        {completed && !editing ? (
          ...
          <span className="flex shrink-0 items-center gap-1">
            <input inputMode="numeric" value={a} onChange={(e) => setA(e.target.value.replace(/[^0-9]/g, "").slice(0, 3))} className="w-11 rounded-[10px] border border-rule-2 bg-surface px-1 py-1.5 text-center text-sm text-ink outline-none focus:border-brand focus:ring-4 focus:ring-brand/15" />
            <span className="text-faint">&ndash;</span>
            <input inputMode="numeric" value={b} onChange={(e) => setB(e.target.value.replace(/[^0-9]/g, "").slice(0, 3))} className="w-11 rounded-[10px] border border-rule-2 bg-surface px-1 py-1.5 text-center text-sm text-ink outline-none focus:border-brand focus:ring-4 focus:ring-brand/15" />
          </span>
        )}
