...
type Team = { id: string; name: string; sport_key: string };
    ...
    <>
      ...
      <aside className="sticky top-0 hidden h-dvh w-64 shrink-0 self-start p-3 md:block">
        <div className="flex h-full flex-col overflow-y-auto rounded-3xl border border-rule/60 bg-white/70 px-3 py-5 shadow-[0_10px_40px_-15px_rgba(10,10,11,0.2)] backdrop-blur-2xl backdrop-saturate-150">
          <div className="flex items-center gap-3 rounded-2xl border border-[#ffd9cb] bg-tint-brand p-3">
            ...
            {items.map(({ href, label, Icon, exact }) => {
              ...
              return (
                <Link
                  ...
                  className={`flex h-11 items-center gap-3 rounded-2xl px-3 text-sm font-semibold transition-colors ${active ? "bg-tint-brand text-brand-deep" : "text-ink hover:text-brand-deep"}`}
                >
          ...
          <div className="mt-6 border-t border-rule/60 pt-3">
            {others.length > 0 ? (
              <>
                <p className="kicker mb-1 px-3 text-faint">Switch team</p>
                {others.map((t) => (
                  <Link key={t.id} href={`/team/${t.id}`} className="flex h-9 items-center gap-2.5 rounded-xl px-3 text-sm font-medium text-ink transition-colors hover:bg-bg">
            ...
            <Link href="/me" className="lift flex items-center gap-2.5 rounded-2xl bg-black/[0.04] p-2 transition-colors hover:bg-black/[0.07]">
              <Avatar url={personal.url} hue={personal.hue} name={personal.name} size={28} ring />
              <span className="min-w-0 flex-1 text-left">
                <span className="block truncate text-sm font-semibold text-ink">{personal.name}</span>
                <span className="block text-xs text-mute">{roleLabel} · back to personal</span>
              </span>
              <ChevronLeft size={15} className="shrink-0 text-faint" />
            </Link>
          </div>
        </div>
      </aside>
