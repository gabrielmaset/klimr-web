...
import {
  LayoutDashboard, IdCard, Users, Handshake, ShieldCheck, Globe,
  ChevronDown, ChevronLeft, ChevronRight, Briefcase,
} from "lucide-react";
import { Avatar } from "@/components/avatar";
import { kindLabel, TIER_LABEL, BUSINESS_STATUS_LABEL } from "@/lib/business";

type Business = { id: string; slug: string; name: string; kind: string; verification_level: string; status: string };
...
/** The business portal rail — same grammar as the tournament workspace rail
 *  (Gabriel's model: click the business name in the lower menu, enter a
 *  portal with its own menu). Dark gradient shell, collapse/overlay contract,
 *  glyph header, grouped tools, public-page link, back-to-Klimr pill. */
export function BusinessNav({ business, role, personal }: { business: Business; role: string; personal: Personal }) {
  ...
  const groups: { header?: string; items: Item[] }[] = [
    { items: [{ href: base, label: "Dashboard", Icon: LayoutDashboard, exact: true }] },
    {
      header: "Manage",
      items: [
        { href: `${base}/profile`, label: "Profile", Icon: IdCard },
    ...
    },
    {
      header: "Sponsorship",
      items: [
        { href: `${base}/sponsorships`, label: "Sponsorships", Icon: Handshake },
  ...
  useEffect(() => {
    const saved = window.localStorage.getItem("klimr.bizrail");
  ...
  const toggleRail = () => {
    ...
    setRailStored(next);
    window.localStorage.setItem("klimr.bizrail", next ? "1" : "0");
  };
  const closeOverlay = () => {
  ...
  const renderItem = ({ href, label, Icon, exact }: Item) => {
  ...
  return (
    <>
      ...
      >
        ...
        <div
          className={`relative flex flex-col overflow-hidden rounded-3xl border border-[#173a2c] bg-[linear-gradient(180deg,#08301f,#04180f)] shadow-[0_10px_40px_-15px_rgba(10,10,11,0.5)] ${
            overlayMode && overlayOpen ? "absolute inset-y-3 left-3 z-10 w-[232px] shadow-e3" : "h-full w-auto"
          }`}
        >
          <div className="relative min-h-0 flex-1">
            <div ref={scrollRef} className={`flex h-full flex-col overflow-y-auto py-5 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden ${overlayMode && overlayOpen ? "px-3" : collapsed ? "px-1.5" : "px-3"}`}>
              <div className={`rounded-2xl border border-rail-border bg-white/[0.05] ${collapsed ? "p-1.5" : "p-3"}`}>
                <div className={`flex items-center ${collapsed ? "justify-center" : "gap-3"}`}>
                  <span className={`grid shrink-0 place-items-center rounded-2xl bg-white text-[#08301f] ${collapsed ? "h-10 w-10" : "h-11 w-11"}`} title={collapsed ? business.name : undefined}>
                    <Briefcase size={collapsed ? 22 : 26} />
                  </span>
                  {collapsed ? null : (
                    <div className="min-w-0">
                      <p className="kicker text-rail-muted">{kindLabel(business.kind)}</p>
                      <p className="text-sm font-bold leading-snug text-rail-fg line-clamp-2 [overflow-wrap:anywhere]">{business.name}</p>
                    </div>
                  )}
                </div>
                {collapsed ? null : (
                  <span className="mt-2.5 inline-flex flex-wrap gap-1.5">
                    <span className="rounded-full bg-white/10 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-rail-fg">
                      {BUSINESS_STATUS_LABEL[business.status] ?? business.status}
                    </span>
                    <span className="rounded-full bg-white/10 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-rail-fg">
                      {TIER_LABEL[business.verification_level] ?? business.verification_level}
                    </span>
                  </span>
                )}
              </div>
            ...
            <div
              ...
              style={{ background: "linear-gradient(to bottom, transparent, #05180f 85%)" }}
            >
      ...
      <header className="print:hidden pt-safe px-safe sticky top-0 z-40 border-b border-rule bg-surface md:hidden">
        <div className="flex items-center gap-3 px-4 py-2.5">
          <span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-tint-brand text-brand-deep"><Briefcase size={20} /></span>
        ...
        <nav className="flex gap-1 overflow-x-auto px-3 pb-2" aria-label="Business">
          {allItems.map(({ href, label, Icon, exact }) => {
            const active = isActive(href, exact);
            return (
              <Link
                key={href}
