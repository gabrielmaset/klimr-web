...
export function SponsorsEditor({ tournamentId, initial }: { tournamentId: string; initial: Sponsor[] }) {
  ...
  async function uploadOne(file: File): Promise<string | null> {
    if (!file.type.startsWith("image/")) throw new Error("Images only (JPG, PNG, or WebP).");
    if (file.size > 8 * 1024 * 1024) throw new Error("Each image must be under 8 MB.");
    const supabase = createClient();
    const signed = await createGalleryUploadUrl(tournamentId, file.type);
  ...
  async function onLogo(id: string, e: React.ChangeEvent<HTMLInputElement>) {
  ...
  async function onPhotos(id: string, e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (added.length) setRows((rs) => rs.map((r) => (r.id === id ? { ...r, photos: [...r.photos, ...added].slice(0, MAX_PHOTOS) } : r)));
    } catch (e2) {
      setErr(e2 instanceof Error ? e2.message : "Upload failed.");
    } finally {
      setUploading(null);
                ...
                <div className="mt-3">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <label className="press inline-flex cursor-pointer items-center gap-1.5 rounded-xl border border-rule bg-bg px-3 py-2 text-sm font-semibold text-ink transition hover:border-brand">
                      ...
                      <input type="file" accept="image/*" className="hidden" onChange={(e) => onLogo(r.id, e)} />
                    </label>
                  </div>
                </div>
