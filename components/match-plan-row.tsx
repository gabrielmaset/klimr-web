  ...
  return (
    <div className="grid items-center gap-2 rounded-xl border border-rule bg-bg/40 px-3.5 py-2.5 sm:grid-cols-[1fr_auto]">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <input type="datetime-local" value={dt} onChange={(e) => { setDt(e.target.value); setSaved(false); }} className="rounded-lg border border-rule bg-surface px-2.5 py-1.5 text-sm text-ink outline-none focus:border-brand" />
        <input value={crt} onChange={(e) => { setCrt(e.target.value); setSaved(false); }} placeholder="Court" className="w-24 rounded-lg border border-rule bg-surface px-2.5 py-1.5 text-sm text-ink outline-none placeholder:text-faint focus:border-brand" />
